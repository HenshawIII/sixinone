...
export function HomeTwoFacesSection() {
  return (
    <section className="section-soft pt-0 bg-black!">
      <div data-reveal className="mx-auto w-full max-w-full overflow-hidden rounded-t-[2.25rem] bg-white">
        <div data-reveal className="mx-auto grid max-w-[90%] gap-8 px-6 py-12 sm:px-8 md:grid-cols-6 md:items-start md:gap-10 md:px-12 md:py-16 md:pt-32 lg:px-16">
          <p className="col-span-2 pt-2 font-heading text-xs font-normal tracking-[0.005em] text-site-muted md:text-2xl">Who We Are</p>

          <div className="col-span-4">
            <h2 className="max-w-5xl font-heading text-3xl leading-[1.1] text-site-text sm:text-4xl lg:text-5xl">
              We are one brand with two faces: <span>Entertainment</span> and{" "}
              <span className="text-site-muted">Publishing</span>.
            </h2>
            <p className="mt-4 max-w-3xl font-body text-base leading-relaxed text-site-muted sm:text-lg">
          ...
          <div className="w-full overflow-hidden">
            <div className="home-image-slider-track flex flex-nowrap items-stretch gap-5">
              {[...sliderImages, ...sliderImages].map((image, index) => (
                <article
                  className="relative h-[88vh] w-[88vw] shrink-0 overflow-hidden rounded-2xl sm:h-72 sm:w-[620px] lg:h-[70vh] lg:w-[700px]"
                  key={`${image.src}-${index}`}
                >
        ...
        <div data-reveal className="grid  max-w-[90%] mx-auto sm:grid-cols-2 lg:grid-cols-4 py-10">
          <StatItem
